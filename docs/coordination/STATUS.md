...
- 현재 검증 대기: `DF-SG019-01` — line 266 keyboard/Space physical-scroll
  observation과 detached-macOS line 661 CVDisplayLink freeze를 구분해 Antigravity가
  재현·정합화한 뒤 clean Pages-base Chromium 16/16 3회와 SG-019 전체 verify 재실행
...
- SG-019: base `74fe266`에서 persisted lastDifficulty의 router/DOM 복원, UI의
  ...
  그 Phaser freeze를 detached-macOS 환경 결함으로 격리했다. line 266과 line 661의
  인과관계는 입증되지 않았고 필수 Pages-base 전체 통과 증거도 없으므로, 제품
  patch에 우회를 넣지 않은 채 `DF-SG019-01`(medium, integration-blocking)을
  Antigravity에 유지하고 SG-019는 `blocked`다.
