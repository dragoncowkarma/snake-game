...
| SG-013 | P1 | menu/ready/playing/gameOver/won DOM UI, focus 전이, 난이도/점수/재시작, `aria-live` 연결(SG-006이 사람 직접 승인으로 선구현, `docs/coordination/handoffs/SG-006.md` 참고); SG-013은 `src/ui/contracts.ts`를 실제 `src/domain` re-export로 교체하고, hand-authored fixture 대신 실제 `reset`/`enqueueDirection`/`step` 산출물로 focus/aria-live 종단 증거를 보강 | Claude Sonnet 5 / 높음 | SG-006, SG-010 | `src/ui/**`, `src/styles.css` | AC-U01~U09 수동/자동 증거 | Antigravity Gemini 3.1 Pro high | P-CLAUDE-UI |
