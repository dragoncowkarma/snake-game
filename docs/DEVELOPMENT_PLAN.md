...
| `READY` | Right/Up/Down, Mute | Right는 현재 방향으로, Up/Down은 회전해 `PLAYING`; Left는 역방향이라 거부, Enter/Space는 무시 |
...
- `AC-U06`: Start/Resume 뒤 보드, Pause 뒤 Resume, 종료 뒤 Restart, Menu 진입 뒤 Start로 focus가 이동하고, 키보드 전체 흐름에서 원치 않는 page scroll이 0이며 버튼 한 번 활성화는 command 한 개만 만든다.
...
1. `READY`에서 Enter/Space는 무시되어 정지하고, Left는 역방향으로 거부되며(반대 방향 거부와 같은 분기), Right/Up/Down은 각각 명시된 방향으로 시작한다.
