...
export function createGameShell(
  ...
  // (src/game/input-controller.ts), and for every key that maps to a handled shell
  // shortcut command it suppresses this listener first — its board-focused handler
  // calls stopPropagation() and its document capture-phase handler calls
  // stopImmediatePropagation() for the same shortcut set — so this listener never
  // actually dispatches those commands. Do not remove either suppression call in
  // InputController without also retiring or gating this listener, or those commands
  // will dispatch twice (violates AC-U06's one-activation-one-command contract).
